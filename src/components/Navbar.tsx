...
import { UserRole, MerchantSettings, getThemeClasses } from '../types';
...
export default function Navbar({
  ...
  return (
    <header className="sticky top-0 z-40 w-full" id="global-shoestore-navbar">
      ...
      <div className="w-full bg-white/95 backdrop-blur-md border-b border-gray-150">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => { onChangeView('store'); onSetCategory('all'); }}>
            <div className={`w-12 h-12 ${t.bg} hover:opacity-90 rounded-2xl flex items-center justify-center text-white shadow-md transition-transform hover:scale-105`}>
          ...
          <div className="flex items-center gap-3">
            ...
            {currentView === 'store' && (
              ...
              >
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-red-505 border-2 border-white text-white text-[10px] font-black w-6 h-6 rounded-full flex items-center justify-center shadow-md animate-bounce">
                    {cartCount}
                  </span>
                )}
              </button>
      ...
      {currentView === 'store' && (
        <div className="md:hidden w-full bg-white/95 border-b border-gray-100 py-2.5 px-4 overflow-x-auto flex gap-2">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => onSetCategory(cat.id)}
